...
class PmergeMe
{
    public:
        ...
        PmergeMe &operator=(const PmergeMe &other) {if (this != &other) {this->_arr = other._arr;} return *this;}
        ~PmergeMe() {}
...
template<typename C>
void PmergeMe<C>::swap(elem_t &a, elem_t &b) {
	elem_t tmp = a;
	a = b;
	b = tmp;
}
...
template<typename C>
size_t PmergeMe<C>::jacobsthal(size_t k) const {
	if (k == 0 || k == 1) return k;
	size_t a = 0, b = 1, c;
...
template<typename C>
void PmergeMe<C>::assign_label(C& arr, const size_t elem_size)
{
...
// [b1, a1], [b2, a2], [b3, a3] , .... 
template<typename C>
void PmergeMe<C>::sort_pairs(C& arr, size_t size)
{
	Citer a, b;
...
template<typename C>
void PmergeMe<C>::split_leaders(C& arr, C& main, C& pend, size_t size)
{
	size_t nb_pairs = arr.size() / (size * 2);
...
void PmergeMe<C>::update_pair_index(C& main, C& pend, size_t inserted_index, size_t size)
{
	...
	// Get to b(n/2) from a(n/2) wich are the last element in main to get to the last unpaired elements of pend
...
template<typename C>
void PmergeMe<C>::assign_pair_index(C& main, C& pend, size_t size)
{
	size_t i = 0;
	int index = 0;
	Citer a = main.begin();
...
template<typename C>
void PmergeMe<C>::binary_insertion(C& main, C& pend, size_t size) 
{
...
template<typename C>
void PmergeMe<C>::merge_insertion(C& arr, size_t size)
	{
	size_t nb_pairs = arr.size() / (size * 2);
